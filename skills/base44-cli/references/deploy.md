...
4. **Connectors** - All connector configurations in `base44/connectors/` (with OAuth authorization flow)
...
5. Deploys all resources in sequence:
   ...
   - Pushes connector configurations (with OAuth authorization flow if needed)
   - Uploads site files
...
## OAuth Authorization for Connectors

If any connectors require OAuth authorization:

1. The CLI displays authorization URLs for connectors needing OAuth
2. You're prompted to authorize in the browser (unless `-y` or `CI` env is set)
3. OAuth authorization flow runs for each connector
4. Connectors that need authorization but aren't authorized will be reported

In CI/CD environments (`CI` environment variable set), OAuth prompts are skipped and authorization links are displayed for manual completion.

## Notes

- If no resources are found, the command exits with a message
- Use individual commands (`entities push`, `functions deploy`, `agents push`, `connectors push`, `site deploy`) if you only want to deploy specific resources
- The site must be built before deployment - this command does not run `npm run build` for you
- Connector OAuth authorization may require browser access - in CI/CD, authorization links are displayed for manual completion
...
| `base44 connectors push` | Push only connectors (with OAuth authorization) |
